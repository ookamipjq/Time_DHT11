...
void OLED_ShowChar(unsigned char x,unsigned char y,unsigned char ch,unsigned char TextSize)
{
	...
	switch(TextSize)
	{
		...
		case 2:
		{
			...
			OLED_SetPos(x,y);
			for(i=0;i<8;i++)
					WriteDat(F8X16[ch1*16+i]);
				OLED_SetPos(x,y+1);
				for(i=0;i<8;i++)
					WriteDat(F8X16[ch1*16+i+8]);
		} break;
	}
}
...
//--------------------------------------------------------------
// Prototype      : OLED_ShowNum(unsigned char x,unsigned char y,unsigned int num,unsigned char len,unsigned char TextSize)
// Calls          : 显示数字
	...
	switch(TextSize)
	{
		case 1:
		{
			for(t = 0;t<len;t++)
			{
				...
				if(temp == 0)
				{
					OLED_ShowChar(x+(8/2+2+2)*t,y,'0',1);
//					OLED_ShowChar(x+(8/2+2+2)*t,y,'0',1);
				}
				else 
				{
					OLED_ShowChar(x+(8/2+2+2)*t,y,temp+'0',1);
//					OLED_ShowChar(x+(8/2+2+2)*t,y,temp+'0',1);
				}
			}
		} break;
		case 2:
		{
			for(t = 0;t<len;t++)
			{
				...
				if(temp == 0)
				{
					OLED_ShowChar(x+(16/2)*t,y,'0',2);
//					OLED_ShowChar(x+(16/2+2)*t,y,'0',2);
				}
				else 
				{
					OLED_ShowChar(x+(16/2)*t,y,'0'+temp,2);
//					OLED_ShowChar(x+(16/2+2)*t,y,'0'+temp,2);
				}
			}
		} break;
	}
